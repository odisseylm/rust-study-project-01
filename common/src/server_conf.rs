...
pub fn get_server_port(service_name: &str) -> Result<u16, anyhow::Error> {
    ...
    */
    ...
    let port_env_vars = [
        &format!("{}_SERVER_PORT", service_name.to_uppercase()),
        &format!("{}SERVER_PORT", service_name.to_uppercase()),
        "SERVER_PORT",
        "PORT",
    ];

    let str_port = port_env_vars.iter()
        .map(|env_name| env_var(env_name))
        .find_or_first(|res|{
            let res = res.as_ref();
            // skip 'Ok(None) => env var not found'
    ...
    match str_port {
        ...
        Some(ref str_port) => {
            ...
            let port: u16 = FromStr::from_str(str_port)
                .map_err(|_|anyhow!("XXX_SERVER_PORT value [{str_port}] has wrong format.")) ?;
            Ok(port)
        }
    }
}
