...
pub fn pg08_db_connection_options(connection_type: ConnectionType, app_name: &str)
                                  -> Result<Option<sqlx_postgres_08::PgConnectOptions>, anyhow::Error> {
    ...
    let mut options = sqlx_postgres_08::PgConnectOptions::new()
    ...
    if let ConnectionType::Ssl = connection_type {
        ...
        options = options
            ...
            .ssl_mode(sqlx_postgres_08::PgSslMode::VerifyFull)   // requires usage ca.crt.pem
            // ? Why not 'server' cert ?
            //
...
pub fn pg08_db_connection(app_name: &str, connection_type: ConnectionType) -> Result<sqlx_postgres_08::PgPool, anyhow::Error> {
    match connection_type {
        ConnectionType::Plain =>
            pg08_db_plain_connection(app_name),
        ConnectionType::Ssl =>
            pg08_db_ssl_connection(app_name),
        ConnectionType::Auto =>
...
fn pg08_db_plain_connection(app_name: &str) -> Result<sqlx_postgres_08::PgPool, anyhow::Error> {
    let options = pg08_db_connection_options(ConnectionType::Plain, app_name) ?;
    let options = options.ok_or_else(||anyhow!("No Postgres DB connection options.")) ?;
    Ok(sqlx_postgres_08::PgPool::connect_lazy_with(options))
}


fn pg08_db_ssl_connection(app_name: &str) -> Result<sqlx_postgres_08::PgPool, anyhow::Error> {
    let options = pg08_db_connection_options(ConnectionType::Ssl, app_name) ?;
    let options = options.ok_or_else(||anyhow!("No Postgres DB connection options.")) ?;
    Ok(sqlx_postgres_08::PgPool::connect_lazy_with(options))
}


fn pg_db_auto_type_connection(app_name: &str) -> Result<sqlx_postgres_08::PgPool, anyhow::Error> {
    match env_var_static(POSTGRES_SSL_CERT_PATH) ? {
        None => pg08_db_plain_connection(app_name),
        Some(_cert_path) => pg08_db_ssl_connection(app_name),
    }
}
